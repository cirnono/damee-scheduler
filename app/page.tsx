...
export default function Home() {
    ...
    const activePlan =
        PLAN_OPTIONS.find((plan) => plan.id === activePlanId) ??
        PLAN_OPTIONS[0];
    ...
    function handleGenerateSchedule() {
        const generatedCells = generateSchedule(
            activeEmployees,
            activeStoreRoles,
            {
    ...
    function handleMoveScheduleEmployee({
        ...
        setSchedulePlans((current) => {
            ...
            const otherCells = currentPlan.cells.filter((cell) => {
                const isFromCell =
                    cell.day === fromDay && cell.role === fromRole;
    ...
    function restoreFromBackup(backup: SchedulerBackup) {
        ...
        const nextStoreId = storeExists
            ? backup.activeStoreId
            : DEFAULT_STORE_ID;
    ...
    async function handleImportFullBackupFromFile(file: File) {
        ...
        try {
            ...
            setBackupMessage(
                error instanceof Error ? error.message : "导入文件失败",
            );
        }
    }
        ...
        <main className="min-h-screen bg-neutral-950 text-neutral-50">
            <section className="mx-auto max-w-7xl px-6 py-8">
                        ...
                        <div className="flex flex-wrap gap-2 rounded-xl border border-neutral-700 bg-white p-1">
                            {PLAN_OPTIONS.filter(
                                ...
                                <button
                                    key={plan.id}
                                    onClick={() =>
                                        handleCopyCurrentPlanTo(plan.id)
                                    }
                ...
                <div className="mb-4 rounded-2xl border border-neutral-800 bg-neutral-900/60 px-4 py-3 text-sm text-neutral-400">
                    ...
                    <span className="text-neutral-100">
                        {
                            activeScheduleCells.filter(
                                (cell) => cell.employeeId,
                            ).length
                        }
                    </span>
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="mb-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                                <button
                                    onClick={() =>
                                        handleCopyScheduleText("wechat-compact")
                                    }
                                ...
                                <button
                                    onClick={() =>
                                        handleCopyScheduleText("by-date")
                                    }
                                    className="rounded-xl border border-neutral-700 px-4 py-3 text-left text-sm text-neutral-200 hover:bg-neutral-900"
                                >
                                    <span className="block font-semibold text-neutral-100">
                                ...
                                <button
                                    onClick={() =>
                                        handleCopyScheduleText("by-employee")
                                    }
                                    className="rounded-xl border border-neutral-700 px-4 py-3 text-left text-sm text-neutral-200 hover:bg-neutral-900"
                                >
                                    <span className="block font-semibold text-neutral-100">
                                    ...
                                    <div>
                                        <input
                                            ...
                                            onChange={(event) => {
                                                ...
                                                void handleImportFullBackupFromFile(
                                                    file,
                                                );
                                            }}
                                        />
                            ...
                            <div className="rounded-xl border border-neutral-800 bg-neutral-900/60 p-4">
                                ...
                                <textarea
                                    ...
                                    onChange={(event) =>
                                        setBackupJsonInput(event.target.value)
                                    }
