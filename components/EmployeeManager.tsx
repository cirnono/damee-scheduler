...
export function EmployeeManager({
    ...
    const filteredEmployees = useMemo(() => {
        ...
        return employeePool
            .filter((employee) => {
                const isInActiveStore = activeStoreEmployeeIdSet.has(
                    employee.id,
                );

                if (filter === "active-store" && !isInActiveStore) return false;
                if (filter === "not-active-store" && isInActiveStore)
                    return false;
    ...
    function addEmployeeIdsToCurrentStore(employeeIds: string[]) {
        setStoreEmployeeIds((current) => {
            ...
            const nextIds = Array.from(
                new Set([...currentIds, ...employeeIds]),
            );
    ...
    function removeAllFilteredEmployeesFromCurrentStore() {
        ...
        setStoreEmployeeIds((current) => {
            ...
            return {
                ...current,
                [activeStoreId]: currentIds.filter(
        ...
        const availableDays = Array.isArray(item.availableDays)
            ? item.availableDays.filter(
                  (day): day is Employee["availableDays"][number] =>
                      weekdayKeys.includes(
                          day as Employee["availableDays"][number],
                      ),
              )
            : [];
    ...
    function importEmployeesFromJson() {
        ...
        try {
            ...
            const importedEmployees = parsed.map((item, index) => {
                ...
                return normalizeImportedEmployee(
                    item as RawEmployeeImport,
                    index,
                );
            });
            ...
            setEmployeePool((current) => {
                const existingIds = new Set(
                    current.map((employee) => employee.id),
                );
    ...
    function generateTestEmployees() {
        const testEmployees: Employee[] = [
            ...
            },
            {
                ...
                preferredRoles: ["卤收银"].filter((role) =>
                    roles.includes(role),
                ),
            ...
            },
            {
                ...
                availableDays: [],
                targetWorkDays: 3,
                capableRoles: ["后厨豆", "后厨切", "后厨炸", "后厨卤"].filter(
                    (role) => roles.includes(role),
                ),
                preferredRoles: ["后厨炸"].filter((role) =>
                    roles.includes(role),
                ),
            ...
            },
            {
                ...
                preferredRoles: ["后厨切"].filter((role) =>
                    roles.includes(role),
                ),
                avoidRoles: [],
                hourlyRate: 32,
            ...
            },
            {
                ...
                preferredRoles: ["后厨豆"].filter((role) =>
                    roles.includes(role),
                ),
        ...
        addEmployeeIdsToCurrentStore(
            testEmployees.map((employee) => employee.id),
        );
        setImportError("");
    }
                ...
                {roles.map((role) => {
                    ...
                    return (
                        <button
                            key={role}
                            onClick={() =>
                                toggleRoleField(employee.id, field, role)
                            }
                            className={
                                checked
            ...
            {filteredEmployees.length === 0 ? (
                ...
                <div className="space-y-4">
                    {filteredEmployees.map((employee) => {
                        const inCurrentStore = activeStoreEmployeeIdSet.has(
                            employee.id,
                        );
                                ...
                                <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                    <div>
                                        ...
                                        <p className="mt-1 text-xs text-neutral-500">
                                            ...
                                            ，可做{" "}
                                            {employee.capableRoles.length}{" "}
                                            个岗位，偏好{" "}
                                            {employee.preferredRoles.length}{" "}
                                            个，避免{" "}
                                            {employee.avoidRoles.length} 个
                                            {typeof employee.hourlyRate ===
                                            "number"
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            <div className="mb-2 flex items-center justify-between gap-3">
                                                ...
                                                >
                                                    {roles.every((role) =>
                                                        employee.capableRoles.includes(
                                                            role,
                                                        ),
                                                    )
                                                        ? "取消全选"
                                        ...
                                        <div className="rounded-xl border border-neutral-200 bg-neutral-50 p-3">
                                            ...
                                            >
                                                ...
                                                <span className="rounded-full border border-neutral-300 px-2.5 py-1 text-xs text-neutral-500">
                                                    {expandedPreferenceEmployeeIds.includes(
                                                        employee.id,
                                                    )
                                                        ? "收起"
                                                        : "展开"}
