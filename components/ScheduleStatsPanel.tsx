  ...
  return (
    <section className="rounded-2xl border border-neutral-800 bg-neutral-900/70 p-4">
      <div className="mb-3 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="font-semibold text-neutral-100">员工排班统计</h2>
      ...
      {sortedStats.length === 0 ? (
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {sortedStats.map((stat) => {
            const isTargetMode = stat.availabilityMode === "days-per-week";
            const isUnderTarget =
              isTargetMode && stat.assignedDaysCount < stat.targetWorkDays;
            const isOverTarget =
              isTargetMode && stat.assignedDaysCount > stat.targetWorkDays;
            const isOnTarget =
              isTargetMode && stat.assignedDaysCount === stat.targetWorkDays;
              ...
              >
                <div className="mb-3 flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-neutral-500">
                      {stat.assignedDaysCount} 天 / {stat.assignedShiftsCount}{" "}
                      个岗位 / {stat.assignedHours} 小时
                    </p>
                ...
                {isUnderTarget ? (
                  <p className="mb-3 rounded-lg border border-amber-900/70 bg-amber-950/20 px-3 py-2 text-xs text-amber-300">
                    未达到目标，还差{" "}
                    {stat.targetWorkDays - stat.assignedDaysCount} 天。
                  </p>
                ) : null}
                ...
                {isOverTarget ? (
                  <p className="mb-3 rounded-lg border border-red-900/70 bg-red-950/20 px-3 py-2 text-xs text-red-300">
                    超过目标，多了{" "}
                    {stat.assignedDaysCount - stat.targetWorkDays} 天。
                  </p>
                ) : null}
